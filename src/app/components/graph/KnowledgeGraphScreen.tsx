...
import { Search, Maximize2, RotateCcw, Plus, Minus, X, Crosshair, List, FilterIcon, Link } from 'lucide-react';
import {
  edges as allEdges,
  graphNodes,
  edgeTypes,
...
function getFitParams(scope: string, depth: number): { pad: number; minScale: number; maxScale: number } {
  if (scope === 'global') return { pad: 40, minScale: 0.12, maxScale: 0.8 };
  if (scope === 'cluster') return { pad: 80, minScale: 0.3, maxScale: 1.2 };
...
function buildClusterData(kind: NodeKind, nodeIds: string[], edges: typeof allEdges, focusId?: string, hopLevel?: number): GraphCluster {
  ...
  for (const nid of nodeIds) {
    const k = graphNodes.get(nid)?.kind;
    if (k && k in nodeTypeCounts) nodeTypeCounts[k]++;
  }
  ...
  for (const e of edges) {
    if (nodeIds.includes(e.src) || nodeIds.includes(e.dst)) {
      for (const [grp, cfg] of Object.entries(EDGE_GROUPS)) {
        if (cfg.types.includes(e.edgeType)) {
          relGroupCounts[grp] = (relGroupCounts[grp] || 0) + 1;
        }
      }
    }
  }
  const children = nodeIds.slice(0, 50).map((nid) => {
    const n = graphNodes.get(nid);
    if (!n) return null;
    const ec = edges.filter((e) => e.src === nid || e.dst === nid).length;
    return { id: nid, kind: n.kind, label: n.label, edgeCount: ec };
  }).filter(Boolean) as { id: string; kind: string; label: string; edgeCount: number }[];
  const dominantKind = nodeTypeCounts.finding > nodeTypeCounts.question && nodeTypeCounts.finding > nodeTypeCounts.experiment ? 'findings'
    : nodeTypeCounts.question > nodeTypeCounts.experiment ? 'questions' : 'experiments';
  const label = hopLevel ? `Third-hop ${dominantKind}` : `${dominantKind.charAt(0).toUpperCase() + dominantKind.slice(1)}`;

  return { id, label, kind: hopLevel ? 'hop-cluster' : 'global-cluster', nodeIds, nodeCount: nodeIds.length, nodeTypeCounts, internalEdgeCount, externalEdgeCount, relationshipGroupCounts: relGroupCounts, sourceFocusNodeId: focusId, hopLevel, children };
}
...
export function KnowledgeGraphScreen() {
  ...
  const [focusStack, setFocusStack] = useState<{ focusId: string; scope: Mode; depth: number }[]>([]);
  ...
  const sub = useMemo(() => {
    if (exploredCluster) {
      const ids = new Set(exploredCluster.nodeIds);
      const inc = allEdges.filter((e) => ids.has(e.src) && ids.has(e.dst));
      const dist = new Map<string, number>();
      for (const id of exploredCluster.nodeIds) dist.set(id, 0);
      return { nodeIds: ids, edges: inc, dist };
    }
  ...
  const filtered = useMemo(() => {
    ...
    const edges = sub.edges.filter((e) => activeEdgeTypes.has(e.edgeType) && nodeIds.has(e.src) && nodeIds.has(e.dst));
  ...
  const filterChanges = useMemo(() => {
    if (isFilterDefault) return 0; let c = 0;
  ...
  const threeHopClusters = useMemo(() => {
    ...
    return Object.entries(byKind).filter(([, ids]) => ids.length > 0).map(([kind, ids]) => {
      const c = buildClusterData(kind as NodeKind, ids, filtered.edges, focusId, 3);
      c.x = ids.reduce((s, id) => s + (layout.get(id)?.x ?? 0), 0) / ids.length;
      c.y = ids.reduce((s, id) => s + (layout.get(id)?.y ?? 0), 0) / ids.length;
      return c as GraphCluster & { x: number; y: number };
    });
  ...
  const globalClusters = useMemo(() => {
    ...
    return Object.entries(byKind).filter(([, ids]) => ids.length > 0).map(([kind, ids]) => {
      const c = buildClusterData(kind as NodeKind, ids, filtered.edges);
      c.x = ids.reduce((s, id) => s + (layout.get(id)?.x ?? 0), 0) / ids.length;
      c.y = ids.reduce((s, id) => s + (layout.get(id)?.y ?? 0), 0) / ids.length;
      return c as GraphCluster & { x: number; y: number };
    });
  ...
  // Resolve selectedCluster from allClusters when its ID is selected
  useEffect(() => {
    if (selectedCluster && !allClusters.find((c) => c.id === selectedCluster.id)) {
  ...
  const fitView = useCallback(() => {
    ...
    const w = maxX - minX + fitPad * 2 + NODE_W;
    const h = maxY - minY + fitPad * 2 + NODE_H;
  ...
  useEffect(() => { fitView(); }, [depth, scope, focusId, exploredCluster]);

  const centerOn = useCallback((id: string) => {
    const p = layout.get(id); const el = wrapRef.current;
    if (!p || !el) return;
    const ns = Math.max(view.scale, 0.5);
  ...
  const onWheel = (e: React.WheelEvent) => {
    ...
    setView((v) => { const f = e.deltaY < 0 ? 1.12 : 0.89; const s = Math.min(Math.max(v.scale * f, 0.08), 3); const k = s / v.scale; return { scale: s, x: mx - (mx - v.x) * k, y: my - (my - v.y) * k }; });
  };
  ...
  const zoom = (dir: 1 | -1) => {
    const el = wrapRef.current; const cx = (el?.clientWidth ?? 600) / 2; const cy = (el?.clientHeight ?? 400) / 2;
    setView((v) => { const s = Math.min(Math.max(v.scale * (dir > 0 ? 1.15 : 0.87), 0.08), 3); const k = s / v.scale; return { scale: s, x: cx - (cx - v.x) * k, y: cy - (cy - v.y) * k }; });
  };
  ...
  const adjacent = useMemo(() => {
    ...
    for (const e of filtered.edges) { if (e.src === selectedId) ns.add(e.dst); if (e.dst === selectedId) ns.add(e.src); }
    return ns;
  ...
  const searchMatches = useMemo(() => {
    ...
    return [...graphNodes.values()].filter((n) => n.id.toLowerCase().includes(q) || n.label.toLowerCase().includes(q)).slice(0, 8);
  }, [search]);
  ...
  const focusNode = useCallback((id: string) => {
    ...
    if (focusId && focusId !== id) setFocusStack((p) => [...p.slice(-9), { focusId, scope, depth }]);
    setFocusId(id); setScope('neighborhood'); inspectNode(id); setSearch('');
    requestAnimationFrame(() => centerOn(id));
  }, [focusId, scope, depth, centerOn, inspectNode, exploredCluster]);
  ...
  const goBackFocus = useCallback(() => {
    ...
    setFocusId(prev.focusId); setScope(prev.scope as Mode); setDepth(prev.depth);
    setSelectedId(null); setInspectedId(null); setSelectedCluster(null); setExploredCluster(null);
    requestAnimationFrame(() => centerOn(prev.focusId));
  }, [focusStack, centerOn]);
  ...
  // ── Cluster handlers ──────────────────────────────────────────────
  const selectCluster = useCallback((cluster: GraphCluster & { x: number; y: number }) => {
  ...
  const exploreCluster = useCallback(() => {
    ...
    if (focusId) setFocusStack((p) => [...p.slice(-9), { focusId, scope, depth }]);
    setExploredCluster(selectedCluster);
    setSelectedCluster(null);
    setScope('neighborhood'); // treat as neighborhood-like
    requestAnimationFrame(() => fitView());
  }, [selectedCluster, focusId, scope, depth, fitView]);

  const viewClusterAsList = useCallback(() => {
    if (!selectedCluster) return;
    setViewMode('list');
  }, [selectedCluster]);
  ...
  useEffect(() => {
    ...
    const urlState: GraphURLState = {
      view: viewMode,
      scope: exploredCluster ? 'cluster' : scope,
      depth,
      focus: focusId !== NO_FOCUS_PLACEHOLDER ? focusId : undefined,
      cluster: selectedCluster?.id || exploredCluster?.id,
    ...
    const qs = serializeGraphQuery(urlState);
  ...
  const kindCounts = useMemo(() => {
    ...
    for (const id of filtered.nodeIds) { const k = graphNodes.get(id)?.kind; if (k) counts[k]++; }
    return counts;
  }, [filtered.nodeIds]);
  ...
  const sortedList = useMemo(() => {
    ...
    switch (listSort) {
      case 'type': ids.sort((a, b) => { const ka = graphNodes.get(a)!.kind; const kb = graphNodes.get(b)!.kind; if (ka !== kb) return ka.localeCompare(kb); return a.localeCompare(b); }); break;
      case 'label': ids.sort((a, b) => graphNodes.get(a)!.label.localeCompare(graphNodes.get(b)!.label)); break;
      case 'edges': ids.sort((a, b) => { const ca = filtered.edges.filter((e) => e.src === a || e.dst === a).length; const cb = filtered.edges.filter((e) => e.src === b || e.dst === b).length; return cb - ca; }); break;
    ...
    <div className="flex h-full">
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <div className="flex items-center justify-between border-b border-border-subtle px-6 py-2 bg-surface min-h-10">
          ...
          <button type="button" onClick={copyLink} className="flex items-center gap-1 rounded-sm px-2 py-1 font-mono text-[10px] text-text-muted hover:text-text transition-colors" aria-label="Copy link to current graph view" title="Copy link">
            <Link className="size-3" /> Share
          </button>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 border-b border-border-subtle bg-surface px-4 py-1.5 min-h-10">
          ...
          <select value={scope} onChange={(e) => { setScope(e.target.value as Mode); if (e.target.value === 'global') setFocusStack([]); setExploredCluster(null); }}
          ...
          <div className="flex items-center gap-1 ml-auto shrink-0">
            <SegmentedControl compact
              segments={[{ id: 'graph', label: 'Graph', icon: <Crosshair className="size-3" /> }, { id: 'list', label: 'List', icon: <List className="size-3" /> }]}
              ...
              {filterPanelOpen && !isMobile && (
                <><div className="fixed inset-0 z-10" onClick={closeFilterPanel} aria-hidden="true" />
                  <div id="filter-panel" className="absolute right-0 top-full z-20 mt-1 w-[320px] rounded-sm border border-border-strong bg-popover shadow-xl max-h-[70vh] overflow-auto">
                    <FilterPanel activeKinds={activeKinds} activeEdgeTypes={activeEdgeTypes} kindCounts={kindCounts}
                      ...
                      isDefault={isFilterDefault} onClose={closeFilterPanel} nodeKinds={nodeKinds} /></div>
                </>
              )}
            </div>
          </div>
          ...
          <div className="fixed inset-0 z-50 flex flex-col justify-end">
            ...
            <div className="relative max-h-[85vh] w-full rounded-t-sm border-t border-border-strong bg-surface overflow-auto pb-[env(safe-area-inset-bottom)]">
              <FilterPanel activeKinds={activeKinds} activeEdgeTypes={activeEdgeTypes} kindCounts={kindCounts}
                ...
                onToggleEdgeType={(t) => setActiveEdgeTypes((prev) => { const n = new Set(prev); n.has(t) ? n.delete(t) : n.add(t); return n; })}
                onToggleGroup={toggleEdgeGroup}
                onSelectAllKinds={() => setActiveKinds(new Set(DEFAULT_KINDS))} onClearAllKinds={() => setActiveKinds(new Set())}
                onResetToDefault={() => { setActiveKinds(new Set(DEFAULT_KINDS)); setActiveEdgeTypes(new Set(edgeTypes)); }}
                isDefault={isFilterDefault} onClose={closeFilterPanel} nodeKinds={nodeKinds} />
            </div>
          </div>
        )}
        ...
        {exploredCluster ? (
          <div className="flex items-center gap-3 border-b border-border-subtle bg-surface/80 px-4 py-1.5 min-h-9">
            ...
            <span className="size-2 shrink-0 rounded-full" style={{ background: KIND_COLOR.finding }} />
            <span className="font-mono text-[12px] font-medium text-text shrink-0">{exploredCluster.label}</span>
            <span className="truncate text-[12px] text-text-secondary min-w-0">{exploredCluster.nodeCount} nodes</span>
            <span className="shrink-0 font-mono text-[10px] text-text-muted tabular-nums">{exploredCluster.internalEdgeCount} internal · {exploredCluster.externalEdgeCount} external</span>
          ...
          <FocusContextBar focusId={focusId} focusNode={focusNodeData} hopDepth={depth}
        ...
        {viewMode === 'graph' ? (
          <div ref={wrapRef} onWheel={onWheel} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerLeave={onPointerUp}
            ...
            {filtered.nodeIds.size === 0 ? (
              ...
              <svg className="absolute inset-0 size-full" role="img" aria-label="Knowledge graph visualization">
                ...
                <g transform={`translate(${view.x},${view.y}) scale(${view.scale})`}>
                  {/* ── Global & 3-hop clusters (not when exploring) ── */}
                  {!exploredCluster && scope === 'global' && zoomLevel < 0.4 && (globalClusters as (GraphCluster & { x: number; y: number })[]).map((c) => (
                    <g key={c.id} transform={`translate(${c.x},${c.y})`} opacity={0.9} className="cursor-pointer" role="button" tabIndex={0}
                      ...
                      onClick={(e) => { e.stopPropagation(); selectCluster(c); }}
                      onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); selectCluster(c); } }}>
                  ...
                  {(scope !== 'global' || zoomLevel >= 0.4 || exploredCluster) && filtered.edges.map((e, i) => {
                    const a = layout.get(e.src); const b = layout.get(e.dst);
                    if (!a || !b) return null;
                    ...
                    return (<g key={i} opacity={opacity}>
                      <line x1={a.x} y1={a.y} x2={b.x} y2={b.y} stroke={EDGE_COLOR[e.edgeType]} strokeWidth={isSelPath ? 2.5 : emphatic ? 1.5 : 0.8}
                        strokeDasharray={e.edgeType === 'conflict-suspected' ? '5 4' : undefined} markerEnd={zoomLevel >= 0.35 ? `url(#arrow-${e.edgeType})` : undefined} />
                      {zoomLevel > 0.7 && isSelPath && <text x={(a.x + b.x) / 2} y={(a.y + b.y) / 2 - 3} textAnchor="middle" fontSize="7" fontFamily="JetBrains Mono, monospace" fill={EDGE_COLOR[e.edgeType]} opacity={0.85}>{e.edgeType}</text>}
                    </g>);
                  })}

                  {/* ── Nodes ── */}
                  {(scope !== 'global' || zoomLevel >= 0.4 || exploredCluster) && [...filtered.nodeIds].map((id) => {
                    const node = graphNodes.get(id)!; const p = layout.get(id);
                    if (!p) return null;
                    ...
                    if (tier === 'pill') {
                      return (<g key={id} transform={`translate(${p.x - PILL_W / 2},${p.y - PILL_H / 2})`} opacity={nodeOpacity} className="cursor-pointer" role="button" tabIndex={0}
                        aria-label={`${node.kind} ${id}: ${node.label}`}
                        onClick={(e) => { e.stopPropagation(); inspectNode(id); }}
                        onDoubleClick={(e) => { e.stopPropagation(); focusNode(id); }}
                        onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); inspectNode(id); } }}>
                    ...
                    if (tier === 'compact') {
                      return (<g key={id} transform={`translate(${p.x - COMPACT_W / 2},${p.y - COMPACT_H / 2})`} opacity={nodeOpacity} className="cursor-pointer" role="button" tabIndex={0}
                        aria-label={`${node.kind} ${id}: ${node.label}`}
                        onClick={(e) => { e.stopPropagation(); inspectNode(id); }}
                        onDoubleClick={(e) => { e.stopPropagation(); focusNode(id); }}
                        onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); inspectNode(id); } }}>
            ...
            <div className="absolute bottom-4 right-4 flex gap-1 z-10">
              ...
              {!isFilterDefault && (<IconButton icon={RotateCcw} label="Reset filters" onClick={() => { setActiveEdgeTypes(new Set(edgeTypes)); setActiveKinds(new Set(DEFAULT_KINDS)); fitView(); }} />)}
            </div>
          ...
          <div className="min-h-0 flex-1 flex flex-col">
            {exploredCluster && (
              <div className="flex items-center gap-2 border-b border-border-subtle bg-surface/80 px-4 py-2">
                <span className="font-mono text-[10px] text-text-muted">Cluster: {exploredCluster.label} · {exploredCluster.nodeCount} nodes</span>
                <button type="button" onClick={goBackFocus} className="ml-auto font-mono text-[10px] text-text-muted hover:text-text transition-colors">Back to graph</button>
              </div>
            )}
            ...
            <div className="min-h-0 flex-1 overflow-auto" role="region" aria-label="Relationship list">
              {filtered.nodeIds.size === 0 ? (
                ...
                <div className="divide-y divide-border-subtle">
                  {sortedList.map((id) => {
                    const node = graphNodes.get(id)!; const isSel = selectedId === id;
                    ...
                    if (search.trim()) { const idx = node.label.toLowerCase().indexOf(search.toLowerCase()); if (idx >= 0) labelParts = [{ text: node.label.slice(0, idx), highlight: false }, { text: node.label.slice(idx, idx + search.length), highlight: true }, { text: node.label.slice(idx + search.length), highlight: false }]; }
                    return (<button key={id} type="button" onClick={() => inspectNode(id)}
                      className={cn('flex w-full items-start gap-2 px-3 py-2.5 text-left transition-colors hover:bg-surface-2', isSel && 'bg-surface-2')} aria-current={isSel ? 'true' : undefined}>
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-0.5 text-[12px] text-text-secondary">{labelParts.map((p, i) => p.highlight ? <mark key={i} className="bg-brand-muted text-text rounded-sm px-0.5">{p.text}</mark> : <span key={i}>{p.text}</span>)}</div>
                        <div className="mt-1 flex flex-wrap gap-1">{incident.slice(0, 3).map((ed, i) => { const other = ed.src === id ? ed.dst : ed.src; return <span key={i} className="rounded-sm border border-border-subtle bg-surface px-1 py-0.5 font-mono text-[9px] text-text-muted">{ed.edgeType} → {other.replace('experiments/', '')}</span>; })}</div>
                      </div>
                    </button>);
                  })}
                </div>
      ...
      {/* Node/cluster inspector drawer */}
      {selectedCluster && !inspectedId && (
        <ResponsiveInspectorOverlay isOpen={true} onDismiss={() => setSelectedCluster(null)}>
          <ClusterInspector cluster={selectedCluster} onClose={() => setSelectedCluster(null)}
            onExplore={exploreCluster} onViewAsList={viewClusterAsList} />
        </ResponsiveInspectorOverlay>
      )}
      {inspectedId && (() => {
        const node = graphNodes.get(inspectedId);
        if (!node) return null;
        return (<ResponsiveInspectorOverlay isOpen={true} onDismiss={() => setInspectedId(null)}>
          <NodeInspector node={node} incident={filtered.edges.filter((e) => e.src === inspectedId || e.dst === inspectedId)}
            onClose={() => setInspectedId(null)} onFocus={() => focusNode(inspectedId)} onPickNode={focusNode} navigate={navigate} />
        </ResponsiveInspectorOverlay>);
      })()}
    </div>
  );
...
function MiniMap({ nodeIds, layout, rootId, mode }: { nodeIds: Set<string>; layout: Map<string, Pos>; rootId: string; mode: string }) {
  ...
  return (<div className="absolute right-3 top-3 overflow-hidden rounded-sm border border-border-subtle bg-surface/80 backdrop-blur hidden sm:block">
    ...
    <svg width={W} height={H}>{[...nodeIds].map((id) => { const p = layout.get(id); const n = graphNodes.get(id); if (!p || !n) return null; return <circle key={id} cx={p.x * s + ox} cy={p.y * s + oy} r={mode === 'neighborhood' && id === rootId ? 2.5 : 1.5} fill={KIND_COLOR[n.kind]} />; })}</svg>
  </div>);
}
