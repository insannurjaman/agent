...
function buildBoundaryEdges(cluster: GraphCluster, allEdgesData: typeof allEdges, maxEdges = 12): AggEdge[] {
  const extMap = new Map<string, { outbound: Record<string, number>; inbound: Record<string, number> }>();
  for (const e of allEdgesData) {
    const inSrc = cluster.nodeIds.includes(e.src);
    const inDst = cluster.nodeIds.includes(e.dst);
    if (inSrc && !inDst) {
  ...
  for (const [targetId, dirs] of extMap) {
    ...
    for (const group of allGroups) {
      const outC = dirs.outbound[group] || 0;
      const inC = dirs.inbound[group] || 0;
...
// ── History entry with camera snapshot ─────────────────────────────────
...
export function KnowledgeGraphScreen() {
  ...
  // ── Subgraph ──────────────────────────────────────────────────────
  const sub = useMemo(() => {
    if (exploredCluster) { const ids = new Set(exploredCluster.nodeIds); const inc = allEdges.filter((e) => ids.has(e.src) && ids.has(e.dst)); const dist = new Map<string, number>(); for (const id of exploredCluster.nodeIds) dist.set(id, 0); return { nodeIds: ids, edges: inc, dist }; }
  ...
  // ── Aggregate boundary edges for selected cluster ─────────────────
  const boundaryEdges = useMemo(() => {
    if (!selectedCluster) return [];
    return buildBoundaryEdges(selectedCluster, allEdges, 12);
  }, [selectedCluster, allEdges]);
  ...
  // Fit on structural changes — but skip if we need to restore camera
  useEffect(() => { if (!cameraRestorePending) fitView(); }, [depth, scope, focusId, exploredCluster]);
  useEffect(() => { if (!cameraRestorePending) fitView(); }, [fitView]);

  // Camera restoration: runs after layout is ready
  useEffect(() => {
    if (!cameraRestorePending || !cameraRestoreRef.current) return;
    const cam = cameraRestoreRef.current;
    requestAnimationFrame(() => {
      const el = wrapRef.current;
      if (!el) return;
      // Safe fallback: if camera would put content off-screen, center instead
      let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
      for (const id of filtered.nodeIds) { const p = layout.get(id); if (!p) continue; minX = Math.min(minX, p.x); minY = Math.min(minY, p.y); maxX = Math.max(maxX, p.x); maxY = Math.max(maxY, p.y); }
      const contentW = maxX - minX + 100; const contentH = maxY - minY + 100;
      const visibleW = el.clientWidth / cam.scale; const visibleH = el.clientHeight / cam.scale;
      const offscreen = cam.x + visibleW < minX || cam.x > maxX || cam.y + visibleH < minY || cam.y > maxY;
      if (offscreen || contentW > 10000 || contentH > 10000) {
        // Fallback: center on focus node or content
        if (focusId && focusId !== NO_FOCUS_PLACEHOLDER) {
          const p = layout.get(focusId); if (p) { const ns = Math.max(cam.scale, 0.3); setView({ x: el.clientWidth / 2 - p.x * ns, y: el.clientHeight / 2 - p.y * ns, scale: ns }); }
        } else fitView();
      } else {
        setView(cam);
      }
  ...
  }, [cameraRestorePending, filtered.nodeIds, layout, focusId, fitView]);

  const centerOn = useCallback((id: string) => { const p = layout.get(id); const el = wrapRef.current; if (!p || !el) return; const ns = Math.max(view.scale, 0.5); setView({ x: el.clientWidth / 2 - p.x * ns, y: el.clientHeight / 2 - p.y * ns, scale: ns }); }, [layout, view.scale]);

  const onWheel = (e: React.WheelEvent) => { e.preventDefault(); const el = wrapRef.current; if (!el) return; const r = el.getBoundingClientRect(); const mx = e.clientX - r.left; const my = e.clientY - r.top; setView((v) => { const f = e.deltaY < 0 ? 1.12 : 0.89; const s = Math.min(Math.max(v.scale * f, 0.08), 3); const k = s / v.scale; return { scale: s, x: mx - (mx - v.x) * k, y: my - (my - v.y) * k }; }); };
  ...
  const zoom = (dir: 1 | -1) => { const el = wrapRef.current; const cx = (el?.clientWidth ?? 600) / 2; const cy = (el?.clientHeight ?? 400) / 2; setView((v) => { const s = Math.min(Math.max(v.scale * (dir > 0 ? 1.15 : 0.87), 0.08), 3); const k = s / v.scale; return { scale: s, x: cx - (cx - v.x) * k, y: cy - (cy - v.y) * k }; }); };
  ...
  const focusNode = useCallback((id: string) => {
    ...
    const camera = { x: view.x, y: view.y, scale: view.scale };
    if (focusId && focusId !== id) setFocusStack((p) => [...p.slice(-9), { focusId, scope, depth, camera }]);
  ...
  const goBackFocus = useCallback(() => {
    ...
    cameraRestoreRef.current = prev.camera;
    setCameraRestorePending(true);
    syncURL('push');
  }, [focusStack, syncURL]);
  ...
  const exploreCluster = useCallback(() => {
    if (!selectedCluster) return;
    const camera = { x: view.x, y: view.y, scale: view.scale };
    if (focusId) setFocusStack((p) => [...p.slice(-9), { focusId, scope, depth, camera }]);
  ...
  // URL sync for minor changes → replaceState
  useEffect(() => {
    if (!urlInitDone.current) return;
    // Only use replaceState for minor/filter changes — major events call pushState explicitly
    syncURL('replace');
  }, [activeKinds, activeEdgeTypes, depth, viewMode]);
  ...
  const sortedList = useMemo(() => {
    ...
    switch (listSort) { case 'type': ids.sort((a, b) => { const ka = graphNodes.get(a)!.kind; const kb = graphNodes.get(b)!.kind; if (ka !== kb) return ka.localeCompare(kb); return a.localeCompare(b); }); break; case 'label': ids.sort((a, b) => graphNodes.get(a)!.label.localeCompare(graphNodes.get(b)!.label)); break; case 'edges': ids.sort((a, b) => { const ca = filtered.edges.filter((e) => e.src === a || e.dst === a).length; const cb = filtered.edges.filter((e) => e.src === b || e.dst === b).length; return cb - ca; }); break; default: ids.sort((a, b) => a.localeCompare(b)); } return ids;
        ...
        {viewMode === 'graph' ? (
              ...
              <svg className="absolute inset-0 size-full" role="img" aria-label="Knowledge graph visualization">
                ...
                <g transform={`translate(${view.x},${view.y}) scale(${view.scale})`}>
                  ...
                  {selectedCluster && boundaryEdges.map((ae) => {
                    const extNode = graphNodes.get(ae.targetId);
                    const extPos = layout.get(ae.targetId);
                    const clusterPos = { x: (selectedCluster as GraphCluster & { x?: number }).x ?? 0, y: (selectedCluster as GraphCluster & { y?: number }).y ?? 0 };
                    if (!extPos || !clusterPos) return null;
                    const groupColor = ae.group === 'evidence' ? 'var(--brand-primary)' : ae.group === 'resolution' ? 'var(--green)' : ae.group === 'conflict' ? 'var(--error)' : 'var(--text-muted)';
                    return (<g key={ae.id} opacity={0.5} className="cursor-pointer" role="button" tabIndex={0}
                      aria-label={`${ae.count} ${ae.group} relationships ${ae.direction} ${ae.externalNodeLabel || ae.targetId}`}>
                      <line x1={clusterPos.x} y1={clusterPos.y} x2={extPos.x} y2={extPos.y}
                        stroke={groupColor} strokeWidth={Math.min(ae.count * 0.3 + 1, 4)} strokeDasharray={ae.direction === 'mixed' ? '4 3' : ae.direction === 'inbound' ? '2 4' : undefined} markerEnd={ae.direction === 'outbound' ? `url(#arrow-evidence)` : undefined} />
                      {(zoomLevel > 0.5 || ae.count > 5) && (
                        <text x={(clusterPos.x + extPos.x) / 2} y={(clusterPos.y + extPos.y) / 2 - 4} textAnchor="middle" fontSize="7" fontFamily="JetBrains Mono, monospace" fill={groupColor} opacity={0.8}>{ae.group} · {ae.count}</text>
                      )}
                    </g>);
                  })}
                  ...
                  {(scope !== 'global' || zoomLevel >= 0.4 || exploredCluster) && filtered.edges.map((e, i) => {
                    ...
                    return (<g key={i} opacity={opacity}><line x1={a.x} y1={a.y} x2={b.x} y2={b.y} stroke={EDGE_COLOR[e.edgeType]} strokeWidth={isSelPath ? 2.5 : emphatic ? 1.5 : 0.8} strokeDasharray={e.edgeType === 'conflict-suspected' ? '5 4' : undefined} markerEnd={zoomLevel >= 0.35 ? `url(#arrow-${e.edgeType})` : undefined} />{zoomLevel > 0.7 && isSelPath && <text x={(a.x + b.x) / 2} y={(a.y + b.y) / 2 - 3} textAnchor="middle" fontSize="7" fontFamily="JetBrains Mono, monospace" fill={EDGE_COLOR[e.edgeType]} opacity={0.85}>{e.edgeType}</text>}</g>);
                  })}
                  ...
                  {(scope !== 'global' || zoomLevel >= 0.4 || exploredCluster) && [...filtered.nodeIds].map((id) => {
                    ...
                    // Check if this node has an aggregate edge to the selected cluster
          ...
          <div className="min-h-0 flex-1 flex flex-col">
            ...
            <div className="flex items-center gap-2 border-b border-border-subtle bg-surface/80 px-4 py-1.5 min-h-9">
              ...
              {(['default', 'type', 'label', 'edges'] as SortKey[]).map((s) => (<button key={s} type="button" onClick={() => setListSort(s)} className={cn('rounded-sm px-1.5 py-0.5 font-mono text-[10px] transition-colors', listSort === s ? 'bg-brand-muted text-brand' : 'text-text-muted hover:text-text')}>{s === 'default' ? 'ID' : s === 'type' ? 'Type' : s === 'label' ? 'Label' : 'Edges'}</button>))}
